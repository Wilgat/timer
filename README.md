...
On Termux the install dest is `$PREFIX/bin` when that folder exists (already on PATH). The installer **does not** add `$PREFIX/bin` to `.bashrc`. Otherwise it uses `~/.local/bin` and may add that folder to PATH.
...
Timer name: coffee
[OK] Timer 'coffee' started.
```

`start` asks for the timer name; press Enter to use `default`. `stop` / `status` / `kill` / `reset` then show a numbered list of running timers (`0` leaves). `list` does not ask. `9` exits the main list. `timer --debug` shows the same main list. `timer --json` with no command prints JSON help (not the list). Off-TTY empty argv (`curl | sh`) still install-ensures.
