...
**Status**: Active (Version 1.2.3 – `out_menu_choice`; `prompt_ask` not class B)  
...
|-----------------|------|-----------------------------------|
| **A. Inside output SSOT** | Only `out_text`, `out_json`, and `out_json_error` may `printf` to fd 1/2 for **product** human or JSON lines. Nested `printf … \| sed` used only to escape strings for those emitters is part of the same SSOT. | `out_text` level cases (incl. `menu_choice`); `out_json` / `out_json_error` body builders |
| **B. Function return-via-stdout** | A helper may `printf '%s' "$value"` **solely** so callers capture it with `$(…)`. Prefer `printf` over `echo`. Callers must capture; bare top-level invocation must not be used as the user-facing message path. **MUST NOT** class-B-capture a helper that contains `read`. | `inst_self_uninstall_determine_bin`, `util_get_install_bin_path`, `inst_get_version`, `util_resolve_storage`, `util_get_current_shell`, `util_app_ident` (`prompt_ask` is **not** class B — it assigns `PROMPT_ASK_VALUE`) |
