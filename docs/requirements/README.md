...
**Current state (2026-09-09):** **15** live requirement files: class law `requirement-class-software-dev` (`RQ-CLASS-SOFTWARE-DEV`), eleven `requirement-shell-*.md` with `RQ-SHELL-*` IDs (including **`RQ-SHELL-PATH-AND-SHELL-SUPPORT`** and **`RQ-SHELL-CLI-DEFAULT-INTERACTION`**), plus **domain SSOT** `requirement-domain-timer` (`RQ-DOMAIN-TIMER`). Status **Active**. Registry: `index.md` (Requirement-ID + key). Each file has **§1.1 Human-facing**, **Under command line for normal user only**, and **Design-time verification** (TP-IDs + `tests/*` + `reviews/*` only). **Termux** is a named target system (this login only). TTY empty argv is the numbered start list; menu **start** prompts for the timer name (Enter = `default`); **stop** / **status** / **kill** / **reset** pick from a numbered running-timer list; off-TTY empty argv is Type O ensure; `--json` with no command is JSON help. No Type 1/2. Do **not** invent additional requirement paths without a real ownership gap.
