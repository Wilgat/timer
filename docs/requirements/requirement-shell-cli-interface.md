...
| **Version SSOT** | `VERSION` in script config block (product SSOT; currently `VERSION="2.14.1"`) |
...
| **TP-TX-10** heal timer `$PREFIX/bin` PATH pair | `tests/test_cli.sh` — owned by **RQ-SHELL-PATH-AND-SHELL-SUPPORT** | have |
